...
//Bellman-Ford Algorithm, By Tushar, MCA 3rd Sem(40)
...
void BellmanFord(Graph *graph, int src = 0) {
    ...
    for(int i = 0; i < V - 1; i++)
    {
        for(int j = 0; j < E; j++)
        {
            ...
            if(source_distance[source] != INT8_MAX && source_distance[source] + weight < source_distance[dest])
